...
import { getSectionEmployees } from "../../../../lib/api";
...
export default function EmployeeTab({ id }) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["getSectionEmployees", id, params],
    queryFn: () => getSectionEmployees(id, params),
    enabled: !!id,
  });
  ...
  const columns = [
    ...
    },
    {
      key: "role",
      label: "Role",
      render: (row) => <span>{row.position_Name || row.role || row.jobTitle || "N/A"}</span>
    },
    {
      key: "jobNumber",
      label: "Job Number",
    ...
    <div className="pt-4">
      <DataView
        ...
        card={{
          ...
          subtitle: (row) => `Role: ${row.position_Name || row.role || row.jobTitle || "N/A"}`,
          fields: [
            {
              label: "Job Number:",
