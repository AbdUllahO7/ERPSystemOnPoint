...
export default function SalesReturnInvoiceForm() {
  ...
  const { data: invoicePatternsData } = useQuery({
    queryKey: ["getAllInvoicePatterns"],
    queryFn: () => getAllInvoicePatterns({ Type: "Sales_Returns", pageNumber: 1, pageSize: 100 }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data) => createSalesReturnInvoice(data),
    onSuccess: () => {
      ...
      <div className="xl:col-span-2 space-y-6">
        <div className="bg-card p-6 rounded-xl border shadow-sm grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm" {...register("customer_id")}>
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold">Original Invoice Number <span className="text-red-500">*</span></label>
            <Input placeholder="Original Invoice Number" className={errors.original_invoice_number ? "border-red-500" : ""} {...register("original_invoice_number", { required: "Required" })} />
            {errors.original_invoice_number && <span className="text-xs text-red-500">{errors.original_invoice_number.message}</span>}
          </div>
        </div>
