...
import { getOfficeEmployees } from '../../../../lib/api';
...
export default function EmployeeTab({ id }) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["getOfficeEmployees", id, params],
    queryFn: () => getOfficeEmployees(id, params),
    enabled: !!id,
  });
  ...
  const columns = [
    ...
    },
    {
      key: "position",
      label: "Position",
      render: (row) => <span>{row.position || row.role || "N/A"}</span>
    },
    {
      key: "job_Number",
      label: "Job Number",
    ...
    <div className="pt-4">
      <DataView
        ...
        card={{
          ...
          subtitle: (row) => `Position: ${row.position || row.role || "N/A"}`,
          fields: [
            {
              label: "Job Number:",
