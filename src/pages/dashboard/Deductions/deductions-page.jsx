...
import { Eye, Edit, Trash2, Building2, Users as UsersIcon, Receipt } from "lucide-react";
...
import { getDiscounts, deleteDiscount, getEmployees } from "@/lib/api";
...
export default function DeductionsPage() {
  ...
  const columns = [
    ...
    { key: "amount", label: "Amount", render: (row) => <span className="text-red-500 font-medium">-{row.amount}$</span>, sortable: true },
    ...
    <div className="space-y-4">
        ...
        <div className="overflow-hidden">
          <div className="bg-card text-card-foreground p-4 rounded-xl border shadow-sm flex flex-col sm:flex-row items-end gap-4 flex-wrap">
            ...
            {Object.values(filters).some((val) => val !== "") && (
              <Button variant="outline" onClick={() => { setFilters({ EmployeeId: "", DiscountType: "", MinAmount: "", MaxAmount: "", DateFrom: "", DateTo: "" }); setPage(1); }}>
                Clear Filters
              </Button>
            )}
          </div>
      ...
      <DataView
        ...
        card={{
          ...
          fields: [
            { label: "Type:", value: (row) => row.discountType },
            { label: "Amount:", value: (row) => <span className="text-red-500 font-medium">-{row.amount}$</span> },
          ],
        }}
        onRefresh={() => refetch()}
        onPrint={() => window.print()}
        export={{ label: "Export", onClick: () => console.log("export") }}
        addButton={{ label: "Add Deduction", onClick: () => navigate("/dashboard/hr/deductions/add") }}
        columns={columns}
